...
def main(opt, callbacks=Callbacks()):
    ...
    if opt.resume and not check_comet_resume(opt) and not opt.evolve:
        ...
        opt.save_dir = str(
            increment_path(Path(opt.project) / opt.name, exist_ok=opt.exist_ok, disable_increment=True,)
        )
...
if __name__ == "__main__":
    ...
    with open("params.yaml", 'r') as f:
        ...
        opt = yaml.safe_load(f)["train_type"]
        print(opt)
        with open("params.yaml", 'r') as f1: #Opening twice, fix later please

            if(opt["if_train"]):
                data = argparse.Namespace(**yaml.safe_load(f1)["train"])
            else:
                data = argparse.Namespace(**yaml.safe_load(f1)["finetune"])
    ...
    main(data)
        ...
        # print(data)
        # print(data['train_type'])
        # print(dir(data["train"]))
        # #print(type(yaml.safe_load(f)["train_type"]))
        # #print(yaml.safe_load(f))
        # #print(yaml.safe_load(f)["train_type"])
        # opt = data["train_type"]
        # if opt == "train":
        #     train_data = data["train"]
        #     main(train_data)
        # elif opt == "finetune":
        #     fine_tune_data = data(f)["finetune"]
        #     main(fine_tune_data)
